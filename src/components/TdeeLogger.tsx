  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-950 text-white rounded-3xl p-5 relative overflow-hidden flex flex-col justify-between min-h-[145px] transition-colors">
          <div className="absolute top-0 right-0 h-24 w-24 rounded-full bg-orange-500/10 blur-xl"></div>
          <div className="flex justify-between items-start relative z-10">
            <span className="text-[9px] uppercase font-black text-slate-450 tracking-widest font-mono">ADAPTIVE REAL METABOLISM</span>
          ...
          <div className="my-2 relative z-10">
            <span className="text-4xl font-bold font-mono tracking-tight text-orange-450" id="adaptive-tdee-display">
              {Math.round(adaptiveResults.currentTdee)}
            </span>
            <span className="text-[10px] font-bold text-slate-400 block mt-1 uppercase font-mono">kcal / day adaptive baseline</span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col justify-between min-h-[145px] transition-colors">
          <div className="flex justify-between items-start">
            <span className="text-[9px] uppercase font-bold text-slate-400 tracking-widest font-mono">Metabolic Deviation</span>
            <Award className="h-4.5 w-4.5 text-purple-600" />
          </div>
          <div className="my-2 flex items-baseline gap-1.5">
            {deviation === 0 ? (
              <span className="text-4xl font-bold font-mono tracking-tight text-slate-900">0</span>
            ) : (
            ...
            <span className="text-[10px] font-bold text-slate-400 font-mono uppercase">kcal delta</span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col justify-between min-h-[145px] transition-colors">
          <div className="flex justify-between items-start">
            <span className="text-[9px] uppercase font-bold text-slate-400 tracking-widest font-mono">Calibration Score</span>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col justify-between min-h-[145px] transition-colors">
          <div className="flex justify-between items-start">
            <span className="text-[9px] uppercase font-bold text-slate-400 tracking-widest font-mono">Net Weight Change</span>
          ...
          <div className="my-2">
            ...
            <span className="text-[10px] font-bold text-slate-400 block mt-1 uppercase font-mono">across {adaptiveResults.daysAnalyzed} days</span>
          </div>
          <p className="text-[10px] text-slate-500 leading-relaxed font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6">
            <span className="text-[9px] font-bold tracking-widest text-blue-600 uppercase font-mono block">LEDGER METRICS</span>
            ...
            <form onSubmit={handleAddLog} className="flex flex-col gap-4 mt-4" id="add-log-form">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold text-slate-500 font-mono">Logging Date</label>
                <input
                  type="date"
                  value={date}
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] uppercase font-bold text-slate-500 font-mono">Weight ({unitSystem === 'metric' ? 'kg' : 'lbs'})</label>
                  <input
                    type="number"
                    id="log-weight-input"
                    step="0.1"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] uppercase font-bold text-slate-500 font-mono">Calories (kcal)</label>
                  <input
                    type="number"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] uppercase font-bold text-slate-505 font-mono">Diary Notes (optional)</label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                id="save-log-btn"
                className="w-full flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white border border-blue-600 font-bold py-2.5 rounded-xl text-xs cursor-pointer transition-colors"
              >
                <Plus className="h-4.5 w-4.5 text-white stroke-[3px]" /> Save Coordinate Log
              </button>
            </form>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 flex flex-col gap-3">
            <h4 className="text-xs font-bold text-slate-705 uppercase font-mono">Database Options</h4>
            <div className="grid grid-cols-2 gap-3.5">
              <button
                onClick={handleLoadSampleData}
                className="flex items-center justify-center gap-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-900 font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-wide font-mono cursor-pointer"
              >
                <RefreshCw className="h-3.5 w-3.5 text-slate-550" /> Seed Data
              </button>
              <button
                onClick={handleClearLogs}
                disabled={logs.length === 0}
                className="flex items-center justify-center gap-1 bg-white hover:bg-rose-50 border border-slate-200 text-slate-700 hover:text-rose-900 font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-wide font-mono cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-3xl p-6 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row gap-3 justify-between sm:items-center border-b border-slate-100 pb-4">
            <div>
              <span className="text-[9px] tracking-widest uppercase font-bold font-mono text-blue-600 block">METABOLIC CURVES</span>
              <span className="text-base font-bold text-slate-900 uppercase tracking-wide mt-0.5 block">Performance Plots</span>
            </div>
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase font-mono transition-all cursor-pointer ${
                  graphTab === 'weight'
          ...
          <div className="h-72 w-full mt-2">
            {chartData.length === 0 ? (
              <div className="h-full w-full flex flex-col items-center justify-center p-6 gap-3 border border-dashed border-slate-200 rounded-3xl bg-slate-50">
                <CircleAlert className="h-9 w-9 text-orange-500" />
                <div className="text-center">
                  <p className="font-bold text-slate-900 text-xs uppercase font-mono tracking-wider">No logs available in Cache</p>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                  ...
                  <XAxis
                    ...
                    tick={{ fontSize: 9, fontWeight: 700, fill: '#64748b', fontFamily: 'monospace' }}
                    axisLine={false}
                    tickLine={false}
                  />
                  <YAxis
          ...
          {chartData.length > 0 && (
            <div className="mt-4 flex flex-col gap-2.5">
              <span className="text-[10px] uppercase font-bold text-slate-455 font-mono tracking-widest">Ledger Database ({logs.length} days)</span>
              <div className="max-h-[190px] overflow-y-auto overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="w-full text-[11px] text-left text-slate-600">
                  <thead className="bg-slate-50 text-[9px] uppercase font-mono border-b border-slate-200 sticky top-0 font-bold text-slate-500">
                    <tr>
                      ...
                      <th className="px-3 py-2.5 text-right font-bold">Delete</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {chartData
                      ...
                      .reverse()
                      .map((log) => (
                        <tr key={log.id} className="hover:bg-slate-50 font-semibold transition-colors">
                          <td className="px-3 py-2 font-mono font-bold text-slate-700">{log.date}</td>
                          <td className="px-3 py-2 font-mono text-blue-600 font-bold">
                            {log.weight} {unitSystem === 'metric' ? 'kg' : 'lbs'}
                          </td>
                          <td className="px-3 py-2 font-mono text-slate-800 font-bold">{log.cal.toLocaleString()} kcal</td>
                          <td className="px-3 py-2 truncate max-w-[150px] text-slate-500">
                            {log.notes || '—'}
                          </td>
                          <td className="px-3 py-2 text-right">
                            <button
                              onClick={() => handleDeleteLog(log.id)}
                              className="text-slate-405 hover:text-rose-650 transition-colors cursor-pointer"
                              title="Delete log"
                            >
